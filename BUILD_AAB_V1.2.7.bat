...
echo ========================================
echo BUILD AAB V1.2.7 - TaskGo App
echo ========================================
echo.
echo Versão: 1.2.7 (Code: 127)
echo.
echo Mudanças nesta versão:
echo   - Correção: Dropdowns de estado e cidade usando ExposedDropdownMenuBox (funcionando corretamente)
echo   - Correção: Dropdowns funcionando no cadastro normal e no dialog Google login
echo   - Correção: Implementação robusta com ancoragem correta do menu
echo.
echo ========================================
echo.
...
findstr /C:"versionCode = 127" app\build.gradle.kts >nul
if errorlevel 1 (
    echo ERRO: versionCode não está configurado como 127
    pause
    exit /b 1
)

findstr /C:"versionName = \"1.2.7\"" app\build.gradle.kts >nul
if errorlevel 1 (
    echo ERRO: versionName não está configurado como 1.2.7
    pause
    exit /b 1
)

echo Versão confirmada: 1.2.7 (Code: 127)
