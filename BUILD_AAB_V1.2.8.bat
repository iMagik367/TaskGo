...
echo ========================================
echo BUILD AAB V1.2.8 - TaskGo App
echo ========================================
echo.
echo Versão: 1.2.8 (Code: 128)
echo.
echo Mudanças nesta versão:
echo   - Correção: Módulo AppModule criado com providers para Database, DAOs e Repositórios
echo   - Correção: Binding de PreferencesRepository implementado
echo   - Correção: Build do AAB funcionando corretamente
echo.
echo ========================================
echo.
...
findstr /C:"versionCode = 128" app\build.gradle.kts >nul
if errorlevel 1 (
    echo ERRO: versionCode não está configurado como 128
    pause
    exit /b 1
)

findstr /C:"versionName = \"1.2.8\"" app\build.gradle.kts >nul
if errorlevel 1 (
    echo ERRO: versionName não está configurado como 1.2.8
    pause
    exit /b 1
)

echo Versão confirmada: 1.2.8 (Code: 128)
