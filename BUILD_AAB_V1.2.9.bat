...
echo ========================================
echo BUILD AAB V1.2.9 - TaskGo App
echo ========================================
echo.
echo Versão: 1.2.9 (Code: 129)
echo.
echo Mudanças nesta versão:
echo   - Correção: city/state salvos corretamente na raiz do documento Firestore
echo   - Correção: city/state aparecem no app para o usuário que cadastrou
echo   - Correção: city/state aparecem para outros usuários na mesma localização
echo   - Correção: ProfileViewModel salva city/state no UserFirestore
echo   - Correção: PublicUserProfileScreen lê city/state da raiz do documento
echo   - Correção: Leitura de city/state prioriza raiz do documento (Lei 1)
echo.
echo ========================================
echo.
...
findstr /C:"versionCode = 129" app\build.gradle.kts >nul
if errorlevel 1 (
    echo ERRO: versionCode não está configurado como 129
    pause
    exit /b 1
)

findstr /C:"versionName = \"1.2.9\"" app\build.gradle.kts >nul
if errorlevel 1 (
    echo ERRO: versionName não está configurado como 1.2.9
    pause
    exit /b 1
)

echo Versão confirmada: 1.2.9 (Code: 129)
