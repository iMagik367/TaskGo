...
echo ========================================
echo BUILD AAB V1.3.0 - TaskGo App
echo ========================================
echo.
echo Versão: 1.3.0 (Code: 133)
echo.
echo Mudanças nesta versão:
echo   - Padronização completa de Cards (TaskGoCard)
echo   - Design system unificado (DesignConstants)
echo   - TextFields padronizados (TextFieldHelper)
echo   - Correções de formatação e espaçamento
echo   - Melhorias de UX e consistência visual
...
findstr /C:"versionCode = 133" app\build.gradle.kts >nul
if errorlevel 1 (
    echo ERRO: versionCode não está configurado como 133
    pause
    exit /b 1
)

findstr /C:"versionName = \"1.3.0\"" app\build.gradle.kts >nul
if errorlevel 1 (
    echo ERRO: versionName não está configurado como 1.3.0
    pause
    exit /b 1
)

echo Versão confirmada: 1.3.0 (Code: 133)
