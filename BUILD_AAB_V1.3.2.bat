...
echo ========================================
echo BUILD AAB V1.3.2 - TaskGo App
echo ========================================
echo.
echo Versão: 1.3.2 (Code: 135)
echo.
echo Mudanças nesta versão:
echo   - Análise sistemática completa (5 rodadas)
echo   - Correção de todos os imports faltando
echo   - Padronização completa de Cards
echo   - Correção de filtros de localização
echo   - Atualização de Cloud Functions e Rules
echo   - Melhorias de exibição de dados
echo.
echo ========================================
echo.
...
findstr /C:"versionCode = 135" app\build.gradle.kts >nul
if errorlevel 1 (
    echo ERRO: versionCode não está configurado como 135
    pause
    exit /b 1
)

findstr /C:"versionName = \"1.3.2\"" app\build.gradle.kts >nul
if errorlevel 1 (
    echo ERRO: versionName não está configurado como 1.3.2
    pause
    exit /b 1
)

echo Versão confirmada: 1.3.2 (Code: 135)
